...
type Opinion = { id: string; title: string | null; body: string; createdAt: string; viewNonce?: string; events: { type: "SUBMITTED" | "DELIVERED" | "VIEWED"; occurredAt: string }[] };
...
function ViewContent() {
  ...
  useEffect(() => { if (!id) return; fetch(`/api/opinions/${encodeURIComponent(id)}`, { cache: "no-store" }).then(async (response) => { const body = await response.json(); if (!response.ok) throw new Error(body.error?.message ?? "取得できませんでした。"); return body.data as Opinion; }).then(async (data) => { setOpinion(data); if (data.viewNonce) await fetch(`/api/councilor/opinions/${encodeURIComponent(data.id)}/views`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ viewNonce: data.viewNonce }) }); }).catch((reason: Error) => setError(reason.message)); }, [id]);
