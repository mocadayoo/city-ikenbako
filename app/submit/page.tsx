...
export default function SubmitPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(null); setIsSending(true);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const fields = Array.isArray(result.fields) ? result.fields.map((field: { path?: string }) => labels[field.path ?? ""] ?? field.path).join("、") : "";
        setError(fields ? `入力を確認してください：${fields}` : result.error?.message ?? "送信できませんでした。");
        return;
      }
      router.replace(`/submit/complete?id=${encodeURIComponent(result.data.opinionId)}`);
    } catch {
